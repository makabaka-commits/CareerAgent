...
public class KnowledgeService {
    ...
    public List<RetrievedKnowledge> search(String query,String topic,int limit){
        ...
        return store.knowledge.values().stream().filter(c->filter.isEmpty()||c.topic().equalsIgnoreCase(filter)).map(c->{
            Set<String> ct=tokens(c.title()+" "+c.topic()+" "+c.section()+" "+c.content());long hit=tokens.stream().filter(ct::contains).count();
            double score=tokens.isEmpty()?0:(double)hit/tokens.size();return new RetrievedKnowledge(c.id(),c.title(),c.source(),c.section(),c.content(),Math.round(score*100)/100.0);
