...
<template>
  <section class="agent-page">
    ...
    <div ref="messages" class="agent-messages">
      ...
      <article v-for="(message,index) in store.chatMessages" :key="index" :class="['chat-message',message.role.toLowerCase()]"><div class="message-meta"><span>{{message.role==='USER'?'YOU':'STEPWISE'}}</span><small>{{message.status}}</small></div><p>{{message.content||'正在组织可验证的回答…'}}</p><div v-if="message.trace?.length" class="tool-trace"><span v-for="(tool,i) in message.trace" :key="i">↳ {{tool.name}} · {{tool.status}}</span></div><div v-if="message.citations?.length" class="citation-list"><span v-for="citation in message.citations" :key="citation.chunkId">↗ {{citation.title}} / {{citation.section}}</span></div></article>
    </div>
